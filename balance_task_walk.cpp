...
run_balance_task(void)
{
    ...
    switch (which_step) {
        ...
        case ASSIGN_COG_TARGET_R:
            ...
            cog_target.x[_X_] = (count == 0) ? cog_des.x[_X_] + 0.045  : cog_des.x[_X_] + 0.09; // pos: right
        ...
        case ASSIGN_JOINT_TARGET_LIFT_UP_R:
            ...
            if (count == 0)
                ...
                target[L_HFE].th = 0.3;
            target[L_KFE].th = 0.2;//+= 1.2;
            target[L_AFE].th = 0.0;//+= 0.6;// neg means go down
//            target[R_HFE].th -= 0.06; // before decrement: 0.32
            if (count != 0){
                target[R_HFE].th = 0.2; // TODO: check
                target[R_KFE].th = 0.3;
                target[R_AFE].th = 0.1;
            }
            time_to_go = duration;
...
//            target[L_AFE].th = -0.04;// neg means go down
        ...
        case ASSIGN_COG_TARGET_L:
            ...
            // what is the target for the COG?
            bzero((void *)&cog_target,sizeof(cog_target));
            cog_target.x[_X_] = cog_des.x[_X_] - 0.09 ; // pos: right or .095
            cog_target.x[_Y_] = cog_des.x[_Y_] + 0.045; //+ 0.015; // pos: forward
...
//            printf("cog_target %f cog_des %f\n", cog_target.x[_Z_], cog_des.x[_Z_]);
        ...
        case ASSIGN_JOINT_TARGET_LIFT_UP_L:
            ...
            target[R_KFE].th = 0.2; //+= 1.0;
            target[R_AFE].th = 0.0; //+= 0.5;// neg means go down
//            target[L_HFE].th -= 0.06;
            target[L_HFE].th = 0.2; // TODO: check
            target[L_KFE].th = 0.3;
            target[L_AFE].th = 0.1;
            time_to_go = duration;
        ...
        case ASSIGN_JOINT_TARGET_LIFT_DOWN_L:
            ...
            target[R_HFE].th = 0.3; //0.1; //-= 0.5;
            target[R_KFE].th = 0.1; //0.2; //-= 1.0;
            target[R_AFE].th = -0.3; //-0.3; //-= 0.5;// neg means go down
//            target[L_HFE].th -= 0.06;
...
//                freeze();
        ...
        case ASSIGN_COG_TARGET_MIDDLE:
            ...
            // what is the target for the COG?
            bzero((void *)&cog_target,sizeof(cog_target));
            cog_target.x[_X_] = cog_des.x[_X_] + 0.045; // assume it will always be from left to middle
            cog_target.x[_Y_] = cog_des.x[_Y_];
            cog_target.x[_Z_] = cog_des.x[_Z_];
