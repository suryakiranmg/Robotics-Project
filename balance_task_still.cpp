...
// dw 4/8 modified
    ...
    if (time_to_go <= 0) {
      which_step = ASSIGN_JOINT_TARGET_LIFT_UP_R;
						time_to_go = duration;
										printf("MOVE_TO_COG_TARGET_R cog des x= %f, y= %f, z= %f\n", cog_des.x[_X_], cog_des.x[_Y_], cog_des.x[_Z_]);
										printf("MOVE_TO_COG_TARGET_R cog tar x= %f, y= %f, z= %f\n", cog_target.x[_X_], cog_target.x[_Y_], cog_target.x[_Z_]);
...
//    printf("In MOVE_JOINT_TARGET_LIFT_UP\n");
  ...
  case ASSIGN_JOINT_TARGET_LIFT_DOWN_R:
      ...
      which_step = MOVE_JOINT_TARGET_LIFT_DOWN_R;
										printf("ASSIGN_JOINT_TARGET_LIFT_DOWN_R cog des x= %f, y= %f, z= %f\n", cog_des.x[_X_], cog_des.x[_Y_], cog_des.x[_Z_]);
										printf("ASSIGN_JOINT_TARGET_LIFT_DOWN_R cog tar x= %f, y= %f, z= %f\n", cog_target.x[_X_], cog_target.x[_Y_], cog_target.x[_Z_]);
  ...
  case MOVE_JOINT_TARGET_LIFT_DOWN_R:
      ...
      //    printf("In \n");
      if (time_to_go <= 0){
										printf("MOVE_JOINT_TARGET_LIFT_DOWN_R cog des x= %f, y= %f, z= %f\n", cog_des.x[_X_], cog_des.x[_Y_], cog_des.x[_Z_]);
										printf("MOVE_JOINT_TARGET_LIFT_DOWN_R cog tar x= %f, y= %f, z= %f\n", cog_target.x[_X_], cog_target.x[_Y_], cog_target.x[_Z_]);
						    //freeze();
  ...
  case ASSIGN_COG_TARGET_L:
      ...
      // what is the target for the COG?
      // x= 0.062 ( 0.062)   y= 0.015 ( 0.016)   z=-0.126 (-0.124)
						bzero((void *)&cog_target,sizeof(cog_target));
      cog_target.x[_X_] = cog_des.x[_X_] + -0.055*2; // pos: right
  ...
  case MOVE_TO_COG_TARGET_L: // this is for inverse kinematics control
      ...
      if (time_to_go <= 0) {
          ...
          time_to_go = duration;
										printf("MOVE_TO_COG_TARGET_L cog des x= %f, y= %f, z= %f\n", cog_des.x[_X_], cog_des.x[_Y_], cog_des.x[_Z_]);
										printf("MOVE_TO_COG_TARGET_L cog tar x= %f, y= %f, z= %f\n", cog_target.x[_X_], cog_target.x[_Y_], cog_target.x[_Z_]);
          //freeze(); // added for now
      }
  ...
  case ASSIGN_JOINT_TARGET_LIFT_DOWN_L:
      ...
      target[R_HFE].th = 0.0;
      target[R_KFE].th = 0.0;
      target[R_AFE].th = 0.0;// neg means go down
